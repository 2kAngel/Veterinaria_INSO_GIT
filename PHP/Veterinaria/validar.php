...
function validar(&$dni, &$contra, &$error){
    ...
    if($dni=="" || !preg_match("/[0-9]{7,8}[A-Z]/",$dni)){
        ...
        $query="SELECT dniCli, nombreCli, apellidoCli, passwordCli, emailCli
			FROM cliente 
			WHERE dniCli = '$dni'";
    ...
    if($contra != $reg_cliente['passwordCli']){
        $error="Error: Contraseña incorrecta";
        $contra="";
        return false;
    }
    $_SESSION['dni'] = $reg_cliente['dniCli'];
    $_SESSION['nombre'] = $reg_cliente['nombreCli'];
